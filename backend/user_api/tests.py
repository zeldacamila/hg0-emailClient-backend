...
class UserTests(APITestCase):
    def setUp(self):
        self.test_user = User.objects.create_user(username='testuser', email='testuser@domain.com', password='Testpassword123!')
    ...
    def test_signup(self):
        ...
        response = self.client.post(reverse('auth-signup'), data, format='json')
    ...
    def test_signup_with_unmatching_passwords(self):
        ...
        response = self.client.post(reverse('auth-signup'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("Password fields didn't match.", response.data['message']['password'][0])
    ...
    def test_signin(self):
        ...
        response = self.client.post(reverse('auth-signin'), data, format='json')
    ...
    def test_signin_with_incorrect_username(self):
        ...
        response = self.client.post(reverse('auth-signin'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("Invalid email or password. Please, check the input data and try again.", response.data['message'])
    ...
    def test_validate_token(self):
        ...
        response = self.client.post(reverse('auth-signin'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        token_validation_data = { 'token': response.data['data']['access_token']}
        validation_response = self.client.post(reverse('auth-validate-token'), token_validation_data, format='json')
    ...
    def test_validate_token_with_invalid_token(self):
        ...
        self.assertIn("Token is invalid or expired", validation_response.data['message'])